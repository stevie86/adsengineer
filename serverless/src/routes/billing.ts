...
billing.post('/subscriptions', async (c) => {
  ...
  try {
    ...
    const customer = await db
      .prepare('SELECT id FROM customers WHERE stripe_customer_id = ? AND id = ?')
      .bind(customer_id, agency_id)
      .first();
...
billing.post('/subscriptions/:subscription_id/cancel', async (c) => {
  ...
  try {
    // Verify ownership
    const sub = await db
      .prepare('SELECT id FROM subscriptions WHERE stripe_subscription_id = ? AND agency_id = ?')
      .bind(subscription_id, agency_id)
      .first();
    ...
    await db
      ...
      .bind(cancel_at_period_end ? 'canceling' : 'canceled', subscription_id)
      .run();
