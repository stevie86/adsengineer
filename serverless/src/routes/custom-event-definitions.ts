...
customEventDefinitionsRoutes.post('/definitions', authMiddleware(), async (c) => {
  try {
    ...
    const requiredFields = ['event_name', 'display_name', 'description', 'trigger_type', 'trigger_conditions', 'supported_platforms', 'strategic_value'];
    ...
    if (!validTriggerTypes.includes(body.trigger_type)) {
      return c.json({ error: `Invalid trigger_type. Must be one of: ${validTriggerTypes.join(', ')}` }, 400);
    }
    ...
    for (const platform of body.supported_platforms) {
      if (!validPlatforms.includes(platform)) {
        return c.json({ error: `Invalid platform: ${platform}. Must be one of: ${validPlatforms.join(', ')}` }, 400);
      }
    }
    ...
    `).bind(auth.org_id, body.event_name).first();

    if (existing) {
      return c.json({ error: 'Custom event with this name already exists for your organization' }, 409);
    }
    ...
    const newDefinition = await db.prepare(`
...
customEventDefinitionsRoutes.delete('/definitions/:id', authMiddleware(), async (c) => {
  try {
    ...
    if (assignments.count > 0) {
      return c.json({ error: 'Cannot delete custom event that is assigned to sites. Disable it instead.' }, 409);
    }
...
customEventDefinitionsRoutes.get('/sites/:siteId', authMiddleware(), async (c) => {
  try {
    ...
    const siteEvents = await db.prepare(`
      SELECT
        sed.*,
        ced.event_name,
        ced.display_name,
...
customEventDefinitionsRoutes.post('/sites/:siteId/assign', authMiddleware(), async (c) => {
  try {
    ...
    const eventDef = await db.prepare(`
      SELECT * FROM custom_event_definitions
      WHERE id = ? AND (org_id = ? OR org_id = 'system') AND is_active = 1
    `).bind(body.event_definition_id, auth.org_id).first();
...
customEventDefinitionsRoutes.delete('/sites/:siteId/events/:eventDefinitionId', authMiddleware(), async (c) => {
  try {
    const auth = c.get('auth');
    const db = c.get('db');
    ...
    `).bind(auth.org_id, siteId, eventDefinitionId).run();
