...
async function encryptBackup(plaintext: string, keyString: string): Promise<{ ciphertext: string; iv: string }> {
...
async function decryptBackup(ciphertext: string, ivString: string, keyString: string): Promise<string> {
  const encoder = new TextEncoder();
