...
const shopifyPayload = {
  ...
  total_price: '99.99',
  currency: 'USD',
  line_items: [{ product_id: 'prod-1', variant_id: 'var-1', quantity: 2 }],
  order_id: 'order-123',
...
console.assert(
  shopifyResult.event?.userData.email === 'test@example.com',
  'Email should be extracted'
);
console.assert(
  shopifyResult.event?.customData.value === 99.99,
  'Total price should be converted to number'
);
console.assert(shopifyResult.event?.customData.currency === 'USD', 'Currency should be extracted');
console.assert(
  shopifyResult.event?.customData.content_ids?.length === 1,
...
console.log(
  'Shopify adapter wraps transformation in try/catch and returns {success: false, error}'
);
...
console.log(
  '  POST /track/general → generalAdapter (pass-through) → StandardEvent → Facebook CAPI'
);
