...
console.assert(
  result.event?.customData.content_ids?.length === 3,
  'Should extract product IDs and names'
);
console.assert(result.event?.customData.num_items === 3, 'Should calculate total quantity');
console.assert(
  result.event?.customData.order_id === 'order_abc123',
  'Order key should be order_id'
);
...
for (const [wooEvent, expectedEvent] of Object.entries(events)) {
  ...
  console.assert(
    r.event?.eventName === expectedEvent,
    `${wooEvent} should map to ${expectedEvent}`
  );
}
