...
export const CompetitorAnalysisSchema = z.object({
  ...
  recentMoves: z.array(z.object({
    date: z.date(),
    type: z.enum(['product_launch', 'partnership', 'acquisition', 'funding', 'pricing_change', 'hiring']),
    description: z.string(),
...
export class CompetitiveAnalysisService {
  ...
  private async analyzeCompetitiveAdvantages(analysis: Partial<CompetitorAnalysis>): Promise<any[]> {
    return [
      {
  ...
  private calculateThreatLevel(analysis: Partial<CompetitorAnalysis>): 'low' | 'medium' | 'high' | 'critical' {
    return 'medium';
  }
  ...
  private async analyzeOurPosition(
    ...
    return {
      ourPosition: {
        ...
        differentiationFactors: ['Platform compatibility', 'Real-time analytics', 'Easy integration']
      },
      marketGaps: [
        {
  ...
  private categorizeChange(change: any): 'strength' | 'weakness' | 'opportunity' | 'threat' | 'trend' | 'pricing' {
  ...
  private timeframeFromChange(change: any): 'immediate' | 'short-term' | 'medium-term' | 'long-term' {
