...
export const LinkedInProfileSchema = z.object({
  ...
  languages: z.array(z.object({
    name: z.string(),
    proficiency: z.enum(['elementary', 'limited_working', 'professional_working', 'full_professional', 'native'])
  })),
...
export const SalesNavigatorLeadSchema = z.object({
  ...
  buyingSignals: z.array(z.object({
    type: z.enum(['job_change', 'promotion', 'company_expansion', 'funding', 'new_hiring', 'technology_adoption']),
    signal: z.string(),
    detectedAt: z.date(),
  ...
  accountInsights: z.object({
    ...
    buyingStage: z.enum(['awareness', 'interest', 'consideration', 'intent', 'purchase', 'retention']),
    ...
    competitorRelationships: z.array(z.object({
      company: z.string(),
      strength: z.enum(['strong', 'moderate', 'weak']),
      lastContact: z.date()
    }))
  }),
  engagementOpportunities: z.array(z.object({
    type: z.enum(['shared_connection', 'alumni', 'former_colleague', 'shared_group', 'recent_post', 'company_announcement']),
  ...
  contactPreferences: z.object({
    ...
    responseHistory: z.array(z.object({
      channel: z.string(),
      date: z.date(),
...
export const LinkedInSearchCriteriaSchema = z.object({
  ...
  companySizes: z.array(z.enum(['1', '2-10', '11-50', '51-200', '201-500', '501-1000', '1001-5000', '5001-10000', '10001+'])),
  seniority: z.array(z.enum(['unspecified', 'owner', 'partner', 'cxo', 'vp', 'director', 'manager', 'senior'])),
...
export class LinkedInSalesNavigatorService {
  ...
  private calculateLeadScore(profile: LinkedInProfile, insights: any, buyingSignals: any[]): number {
    let score = 0;
  ...
  private getMonthsBetween(startDate: any, endDate: Date): number {
    ...
    return (endDate.getFullYear() - start.getFullYear()) * 12 + 
  ...
  private async getSharedConnections(profileId: string): Promise<any[]> {
    try {
      const response = await this.makeAuthenticatedRequest(`/people/${profileId}/sharedConnections`);
      return response.elements || [];
    } catch (error) {
  ...
  private async analyzeActivityPatterns(activities: any[]): Promise<any> {
    ...
    return {
      ...
      lastActive: recentActivities.length > 0 ? 
