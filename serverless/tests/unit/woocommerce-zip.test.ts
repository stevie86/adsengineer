...
describe('WooCommerce ZIP Generation', () => {
  ...
  it('should contain plugin files', async () => {
    ...
    // The buffer should contain the plugin file name
    const bufferStr = new TextDecoder().decode(zipBuffer.slice(0, Math.min(500, zipBuffer.byteLength)));
    expect(bufferStr).toContain('adsengineer-woocommerce');
  });
});
