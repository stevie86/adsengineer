...
import { page, context, browser } from '../setup/e2e-setup';
...
test.describe('Complete User Journey', () => {
  ...
  test.describe('Agency Signup Flow', () => {
    test('should complete full signup process', async () => {
      ...
      await expect(page.locator('text=Your account has been created and payment processed successfully')).toBeVisible();
  ...
  test.describe('Integration Testing', () => {
    test('should handle Shopify webhook integration', async () => {
      const webhookResponse = await page.request.post('http://localhost:8090/api/v1/shopify/webhook', {
        data: {
          id: 123456789,
